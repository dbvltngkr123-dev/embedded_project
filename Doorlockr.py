...
def unlock_door():
    ...
    # 전역 변수를 수정하지 않으므로 global 선언 불필요
...
def handle_lockdown_mode():
    ...
    """
    # lockdown_end_time을 수정하므로 global 선언 필요
    ...
    # 락다운 해제는 메인 루프에서 처리 (실패 횟수 초기화는 메인 루프 해제 시)
...
if __name__ == "__main__":
    # 이 부분의 'global' 선언이 오류를 일으켰으므로 제거했습니다.
    # 전역 변수는 모듈 레벨에서 바로 사용 가능합니다.
    input_code = ""
    ...
    try:
        ...
        while True:
            ...
            if current_time < lockdown_end_time:
                ...
                # 락다운 상태일 때만 남은 시간을 출력하여 사용자에게 피드백 제공
                # (옵션)
                # print(f"락다운 중... 남은 시간: {remaining}초") 
