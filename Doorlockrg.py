...
# [MODIFIED] 무음 패닉 모드 총 작동 시간 (15초로 변경됨)
SILENT_PANIC_DURATION = 15 
# [UPDATED] 무음 펄스가 지속되는 시간 (10초로 변경됨 - 총 작동 시간보다 짧게 조정)
PANIC_PULSE_SOUND_DURATION = 10 
...
def silent_panic_sequence():
    """
    [MODIFIED] 무음 패닉 모드: 10초 동안 조용한 저주파 펄스를 발생시키며 15초간 비상 신호를 전송합니다.
    (총 15초 작동, 남은 시간 카운트다운 표시)
    """
    global is_panic_mode
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print("!!! [SILENT PANIC ACTIVATED] 무음 패닉 모드 활성화 (1125) !!!")
    # 메시지 업데이트: 15초로 변경된 총 작동 시간 반영
    ...
    try:
        # 2. 아주 미세한 저주파 펄스 피드백 (10초 동안 반복)을 별도 스레드에서 시작
        pulse_thread = threading.Thread(target=play_silent_panic_pulse, args=(PANIC_PULSE_SOUND_DURATION,))
        pulse_thread.start()
        
        # 3. 비상 신호 전송 시뮬레이션 (총 15초) 및 카운트다운 로직
        
        remaining_time = SILENT_PANIC_DURATION
        while remaining_time > 0:
            # 1초마다 남은 시간 출력
            print(f"남은 비상 신호 전송 시간: {remaining_time}초")
            time.sleep(1) 
            remaining_time -= 1
